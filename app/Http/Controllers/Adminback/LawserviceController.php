...
class LawserviceController extends Controller
{
    ...
    function insert_lawservice(Request $request){
        ...
        if ($request->hasFile('law_img')) {
            ...
            $filename = time() . '.' . $request->law_img->extension();
        
            // Move the uploaded file to the correct storage path
            $request->law_img->move(storage_path('app/public/images/'), $filename);
        
            // Create the URL to access the file
            $l_imgPath = $request->getSchemeAndHttpHost() . '/storage/images/' . $filename;
        }
